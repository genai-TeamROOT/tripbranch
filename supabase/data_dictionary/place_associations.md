...
수집 범위는 종로구·중구 파일럿뿐입니다. `areaBasedList1`을 종로구·중구로만 호출했기 때문에 `from_content_id`(기준 관광지)는 항상 이 두 구에 속하고, `to_content_id`(연관 관광지)는 그 두 구의 관광지가 실제로 함께 방문된 다른 구의 장소도 섞여 있는 단방향 데이터입니다. 서울 전역 25개 구 확장과 SCHEDULE/RECOMMEND 파이프라인 연동은 별도 작업입니다.

2026-08-26 기준 698행이며(원본 2,300건 중 양쪽 다 `content_id`로 매칭된 것만 적재, 자기참조 5건·중복 65건 제외), `category`는 `관광지` 440건·`음식` 169건·`숙박` 89건입니다. `from_content_id`는 77개 장소(파일럿 대상 종로구 65곳·중구 29곳 중 자신도 매칭에 성공한 장소만)입니다.
...
- `from_content_id`는 항상 종로구·중구입니다. 다른 구 장소를 기준으로 조회하면 결과가 없는 게 정상이며, 매칭 실패나 버그가 아닙니다(서울 전역 확장 전까지).
- 재수집 정책은 "같은 `base_ym`은 덮어쓰기, 다른 `base_ym`은 이력 보존"입니다. `import_place_associations.py`가 `on_conflict=from_content_id,to_content_id,base_ym` + `resolution=merge-duplicates`로 upsert하므로, 매달 재수집해도 과거 `base_ym` 행은 남습니다.
- 원본 JSONL의 엣지 중 상당수(2,300건 중 1,532건)가 매핑 실패로 제외됐습니다. 대부분 요식업·숙박업 프랜차이즈처럼 `places`에 아직 없는 카테고리라서 그렇습니다(`build_place_association_mappings.py`의 unmatched/out_of_coverage 목록 참고). 이 테이블에 없다고 해서 실제로 연관이 없는 건 아니라, 매칭 커버리지의 한계입니다.
