...
### D-061 — thinking 예산은 모델마다 반대로 걸고, 인텐트 분류에만 적용한다

- 상태: `Accepted`, C 구현 완료
- 배경: `gemini-2.5-flash`는 thinking(내부 추론)이 기본 활성인데 코드 어디에도
  `thinking_config`가 없어 모델 기본값(동적 예산)이 그대로 걸려 있었다. "CoT를
  적용한다"가 아니라 **"이미 켜진 것을 통제한다"**가 실제 과제였다.
- 결정 1 — **1순위 `gemini-2.5-flash`는 `budget=0`.**
- 근거(2026-08-11 실측, 68건 전수): 규칙이 정의된 65건 기준 정확도가 예산 0·512·미설정
  셋 다 64/65로 동일한데, 지연은 미설정이 `budget=0`의 2.4배(중앙값 3,905ms vs
  1,606ms), 꼬리는 8.9배였다. 미설정에서만 thinking이 10초 타임아웃을 넘겨
  `504 DEADLINE_EXCEEDED`가 2건 났다. **얻는 것은 속도와 안정성이지 비용이 아니다** —
  토큰 절감은 13.2%에 그친다(prompt 토큰이 전체의 85%라 thinking을 꺼도 남는다).
- 결정 2 — **폴백 `gemini-2.5-flash-lite`는 `budget=512`.** 1순위와 값이 반대다.
- 근거(2026-08-12 실측): lite는 flash와 달리 **thinking이 기본 꺼져 있다**(미설정과
  `budget=0`의 68건 예측이 한 건도 다르지 않고 지연도 같다). 그 상태에서 65건 중
  57건으로 flash(64건)보다 7건 낮고, **하락 7건 중 6건이 대화 이력에 따라 intent가
  갈리는 케이스**였다(대조쌍 9/12). `budget=512`를 실으면 60/65로 오르고 대조쌍이
  12/12로 완전 회복된다. 지연 대가는 1,027ms → 2,118ms로, flash의 p90 5,327ms보다
  여전히 안정적이다.
- 결정 3 — **`classify_intent`에만 적용한다.** 조건 추출 5종·일정 편성 등 다른 호출은
  예산을 싣지 않는다.
- 근거: 두 실험 모두 `classify_intent` 1단계만 측정했다. `_try_model()`은 모든 LLM
  호출이 지나는 공통 경로라 여기서 무조건 예산을 실으면 **재본 적 없는 호출까지 이
  값이 적용된다.** 일정 편성처럼 여러 장소를 순서·시간으로 엮는 호출은 성격이 달라
  같은 값이 최적이라는 근거가 없다. 측정한 범위에만 적용한다.
- 결정 4 — **목록에 없는 모델에는 아무것도 싣지 않는다**(모델 기본값에 맡긴다).
- 근거: 모델 목록은 `.env`(`LLM_MODEL_NAME`/`LLM_FALLBACK_MODEL_NAMES`)에 있어 코드
  밖에서 바뀐다. 실제로 2026-08-11에 폴백 모델명을 교체했다. 기본값을 `0`으로 두면
  새 모델이 들어왔을 때 lite가 그랬듯 **조용히 낮은 품질로 돈다.**
- 근거 보강(2026-08-12 실측) — **`thinking_budget=0` 자체를 거부하는 모델이 있다.**
  `gemini-3.5-flash-lite`와 `gemini-3.6-flash`는 `0`에 `400 INVALID_ARGUMENT`를
  돌려준다(`512`·`-1`·미설정은 정상). 세대별이 아니라 모델별이다 —
  `gemini-3.1-flash-lite`와 `gemini-3.5-flash`는 `0`을 받는다. 따라서 기본값을 `0`으로
  뒀다면 이 모델을 `.env`에 넣는 순간 **조용한 저하가 아니라 인텐트 분류 전 요청이 400으로
  죽는 전면 장애**가 된다. 결정 4는 그 사고를 막는다.
  동시에 **결정 1의 "`budget=0`"은 모든 모델에 옮길 수 있는 규칙이 아니다.** 1순위 모델을
  교체할 때는 그 모델이 `0`을 받는지부터 확인하고, 안 받으면 예산 표에 다른 값을 넣거나
  목록에서 빼야 한다.
- 남은 것: 512를 실어도 lite는 60/65로 flash(64/65)에 4건 부족하다. 29·33·59는 두
  예산 모두에서 5/5 오답이라 thinking으로 닿지 않고, 17·47은 512가 새로 만든 오답이다
  (flash의 512에서도 같은 성격의 중간 예산 퇴행이 관찰됐다). 폴백은 **첫 발화는
  멀쩡한데 대화가 이어진 뒤 조용히 다른 답이 나가는** 모양으로 저하되므로, 맥락 의존
  요청의 폴백 금지 또는 품질 차이 문서화가 별도로 필요하다.
- 조용한 실패 방지: 상수만 두고 config에 싣는 배선을 빠뜨려도 아무도 모르는 자리라,
  `tests/test_gemini_provider.py`가 `generate_content`에 **실제로 실린 인자**에서
  모델별 예산을 단언한다. 폴백으로 넘어갈 때 예산이 함께 바뀌는지도 한 번의
  `_generate` 안에서 확인한다. 배선을 끊으면 실제로 2건이 실패하는 것을 확인했다.
- 후속 관찰(2026-08-12): `gemini-3.5-flash-lite`가 **아무 예산 설정 없이** 규칙 정의
  65건 중 63건으로 2.5-flash(64건)에 1건 차이고, 지연 중앙값은 852ms(2.5-flash
  1,588ms)·최대 1,119ms(2.5-flash 11,969ms)다. 2.5-lite가 5/5로 틀리던 맥락 의존 7건도
  전부 맞힌다. 다만 `temperature=0.0`에서 **판정이 흔들리는 케이스가 처음 나왔고**
  (68번 1/5), 2단계 조건 추출 품질과 단가는 재지 않았다. 모델 교체는 별건으로 남긴다 —
  이 결정은 예산 정책이지 모델 선정이 아니다.
- 실험 자료: `backend/test_results/intent_cot_2026-08-11/`,
  `fallback_model_ab_2026-08-12/`, `fallback_thinking_2026-08-12/`,
  `model_35_flash_lite_2026-08-12/`, `docs/CoT-실험-인텐트분류-20260811.md`. 측정 도구는
  `backend/scripts/measure_intent_thinking.py`.
- 구현: `app/providers/gemini.py`.
...
| 2026-08-12 | D-061 신설 — thinking 예산을 모델별로 반대로 걸고 `classify_intent`에만 적용(C). 1순위 `gemini-2.5-flash`는 `budget=0`(정확도 동일·지연 2.4배 단축·504 제거), 폴백 `gemini-2.5-flash-lite`는 `budget=512`(lite는 thinking이 기본 꺼져 있어 대화 맥락 의존 판정이 7건 무너지고, 예산을 실으면 대조쌍 9/12→12/12로 회복). 재본 적 없는 호출에 값이 번지지 않도록 `_try_model()`에 `operation`을 전달하고, 목록에 없는 모델에는 아무것도 싣지 않는다. 근거는 68건 전수 실험 3회 |
| 2026-08-12 | D-061 근거 보강 — `thinking_budget=0`을 거부하는 모델(`gemini-3.5-flash-lite`·`gemini-3.6-flash`, 400 INVALID_ARGUMENT)이 실측으로 확인돼 결정 4(목록에 없는 모델에는 싣지 않음)가 전면 장애를 막는다는 점을 명시. 세대별이 아니라 모델별이라 1순위 교체 시 `0` 수용 여부를 먼저 확인해야 한다. 함께 잰 `gemini-3.5-flash-lite`의 인텐트 분류 성적(63/65, 지연 852ms)은 후속 관찰로 기록하고 모델 교체는 별건으로 남김 |
