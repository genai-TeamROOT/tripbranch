...
### D-107 — 지원 지역을 서울 25개 구 전체로 확대한다

- 상태: `Accepted` — 구현 완료.
- 배경: 사용자가 "지금 서비스 지역인 전 구가 다 확대되었으니 적용해달라"고 요청.
  실제로 Supabase `places`에는 이미 서울 25개 구 전부에 활성 장소가 있었다(총
  8,007건). 그런데 지역 판정·검색 필터가 참조하는 `SUPPORTED_DISTRICTS`
  (`app/service_area.py`)는 D-086 이후로 16개 구에서 멈춰 있어, 9개 구(강남·송파·
  영등포·서초·금천·구로·관악·강동·동작, 3,343건 활성 장소)가 이미 DB에 있는데도
  검색·위치 판정 단계에서 걸러지고 있었다.
- 결정:
  1. `SUPPORTED_DISTRICTS`에 9개 구를 추가해 서울 25개 구 전체로 확대했다.
     district_code는 이 9개 구의 실제 활성 장소가 이미 갖고 있던 값을 그대로
     옮겼다(place-sync가 TourAPI `lDongSignguCd`로 채운 값이라 기존 16개 구와
     같은 방식으로 이미 검증된 값이다) — 추측하거나 새로 매핑하지 않았다.
  2. `service_area_landmarks.py`의 `DISTRICT_LANDMARKS`(위치 되묻기 대체 버튼,
     TP-160)에도 9개 구 대표 스팟을 2곳씩 채웠다 — 이 딕셔너리는
     `SUPPORTED_DISTRICTS`와 구 이름이 정확히 일치해야 한다는 assert가 있어,
     하나라도 빠지면 앱이 아예 못 뜬다. 좌표는 기존 15개 구처럼 이미 검증된
     값을 재사용할 수 없어(이 9개 구는 그동안 "아직 밖"인 경계 대조군으로만
     쓰였다) Naver Local Search로 구별 지하철역 2곳씩 새로 조회해 주소가 해당
     구로 확인된 값만 썼다.
  3. `tests/test_service_area.py`의 경계 회귀 테스트를 갱신했다 — 강남역·잠실역·
     천호역·영등포역·구로디지털단지역·노량진역은 그동안 "아직 밖"인 경계
     대조군이었는데 이제 "안"으로 옮겼고, 서초·금천·관악구 대표 지점을 새로
     추가했다. 서울 25개 구 전체가 지원 구가 된 뒤로는 서울 안쪽 경계 자체가
     없어져, 남은 `_OUTSIDE` 대조군은 전부 서울 밖(구리·고양·의정부·부산)이다.
     공식 면적 대조표(`_OFFICIAL_AREA_KM2`)에도 9개 구를 위키백과 infobox
     기준으로 추가했다.
- 근거: 이미 DB에 있는 장소를 지원 목록만 안 따라가 못 보여주는 상태였다 —
  "새 지역을 여는" 결정이 아니라 "이미 연 지역을 코드에 반영만 안 한" 것을
  바로잡는 작업이다.
- 부수 정리: 코드 곳곳에 "지원 16개 구 밖" 현재형 서술과 "강남역·영등포처럼
  지원 구 밖의 실시간 인구 허브" 같은 예시가 남아 있었는데, 강남역·영등포 모두
  이제 지원 구 안이라 예시로서 성립하지 않는다. 실시간 인구·상권 우회 로직
  (`enforce_service_area` 오버라이드, TP-171)의 존재 이유였던 측정값도 다시
  쟀다 — 서울시 실시간 인구 121곳 중 지원 구 밖이 49곳→**2곳**(서울대공원 등
  서울 밖 지점), 실시간 상권 82곳 중 32곳→**0곳**으로 줄었다. 그래도 완전히
  0은 아니라서 오버라이드 자체는 남겨두고, 주석의 예시만 여전히 유효한
  서울대공원으로 바꿨다.
- 채택하지 않은 것: 서울 밖(경기·인천 등)까지 넓히는 것 — 이번 요청은 "이미
  DB에 있는 서울 지역을 반영"이었지, 서비스 지역 자체를 서울 밖으로 넓히자는
  결정이 아니다.
- 검증: 백엔드 `pytest` 3,119건 통과(district 필터·위치 판정 회귀 테스트가
  기존 예시 붕괴로 5건 실패했다가 예시 교체 후 통과), `ruff` 클린. Supabase
  실측 쿼리로 25개 구 전체에 활성 장소가 있음을 재확인.
- 상세: `backend/app/service_area.py`, `backend/app/service_area_landmarks.py`,
  `backend/app/tools/resolve_location.py`, `backend/app/agent_context/service.py`,
  `backend/tests/test_service_area.py`, `backend/tests/test_service_area_landmarks.py`,
  `backend/tests/test_resolve_location_tool.py`, `backend/tests/test_place_provider.py`,
  `backend/tests/test_festival_provider.py`, `backend/tests/agent_context/test_service.py`
...
| 2026-08-30 | D-107 신설 — 지원 지역을 서울 25개 구 전체로 확대. Supabase엔 이미 25개 구 전부에 활성 장소가 있었는데(9개 구 3,343건) `SUPPORTED_DISTRICTS`가 D-086 이후 16개 구에서 멈춰 검색·위치 판정에서 걸러지고 있었다 — 실제 활성 장소의 district_code를 그대로 옮겨 9개 구를 추가했다. `DISTRICT_LANDMARKS`(TP-160 되묻기 대체 버튼)에도 신규 9개 구 대표 스팟을 Naver Local Search로 조회해 채웠다. 경계 회귀 테스트에서 그동안 "아직 밖" 대조군이던 강남역·잠실역·천호역·영등포역·구로디지털단지역·노량진역을 "안"으로 옮기고, "지원 16개 구 밖" 현재형 서술과 강남역·영등포를 미지원 예시로 쓰던 주석·테스트를 갱신(실시간 인구 우회 로직의 근거 수치도 121곳 중 49→2곳, 82곳 중 32→0곳으로 재실측) |
