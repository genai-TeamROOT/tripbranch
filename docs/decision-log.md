...
- 한계: **측정과 제품이 아직 어긋나 있다.** 지금까지 잰 것은 "후보 N곳을 보내고 상위
  ...
  **상위 5곳 기준으로 직접 잰 값이 아니다.** 재측정 뒤 확정한다.

  **3곳은 의도된 값이 아니다.** 사진 검색은 확인 13에서 "제품이 5곳을 보여주니 5곳을
  잰다"로 한 번 맞췄는데(그때 프론트에 `limit: 5`를 넣었다), 확인 15부터 다시 3곳으로
  돌아가 확인 22까지 그대로 갔다. 왜 3인지 적어둔 곳이 없고 앞 스크립트에서 복사된
  값으로 보인다. **"VLM 재랭킹이 통한다"는 방향 자체는 흔들리지 않지만**(뽑히는 곳이
  실제로 바뀌었고 그 변화가 사람 채점에서 이득으로 나왔다) **후보 수 8과 문턱 0.50은
  3곳 기준이다.** 특히 후보 수는 "보여줄 수보다 몇 곳 넉넉한가"가 곧 이득의 크기라
  5곳 기준으로 다시 재야 한다. 잡음 바닥도 함께 재 두었다 — 사진 크기만 다른 두 판이 +6.8%p와
