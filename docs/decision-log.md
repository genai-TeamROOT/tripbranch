...
- 결정 9: **사진을 보낸다. 글로 바꾸는 길은 값이 싸지만 근거가 약하다.** 검색당 26원을
  줄이려고 장소마다 오픈 VLM(Qwen2.5-VL-3B)이 쓴 분위기 설명글을 미리 만들어 두고
  여러 방식으로 비교했다(질의 39장, 같은 후보 10곳).

  | 방식 | 차이 | p | nDCG | 검색당 |
  | --- | --- | --- | --- | --- |
  | 글 + 코사인 | +0.0%p | 0.977 | 0.701 | 0원 |
  | 글 + Gemini 판정 | +3.1%p | 0.219 | 0.795 | 3.2원 |
  | 사진 + 글 (질의만 사진) | +3.6%p | 0.109 | 0.844 | 6.5원 |
  | **사진 + 사진** | **+5.6%p** | **0.006** | **0.883** | 40.0원 |
  | 글 + Qwen2.5-3B 판정 | −0.8%p | 0.754 | — | 3.4원 |

  **코사인이 0이었던 것은 글의 한계가 아니라 비교 방식의 한계였다** — 같은 글을
  Gemini에게 판정시키자 +3.1%p가 나왔다. 다만 **유의한 것은 사진뿐이고**(`p = 0.006`),
  사진을 쓸수록 nDCG가 단조로 올라간다(0.795 → 0.844 → 0.883). 글로 바꾸면 정보를
  잃는 것이 맞고 절반쯤 남는다. 6,829곳의 설명글을 미리 만들어야 하는 부담도 있어
  이번에는 사진을 보낸다. **비용이 걸림돌이 되면 다시 꺼낼 카드다.**

  **오픈 모델은 판정을 못 한다.** `Qwen2.5-3B`는 임베딩 순서보다도 못했고 29장 중
  4장은 답 형식조차 못 맞췄다(3B는 응답 스키마를 걸 수 없다). 확인 21에서 `flash-lite`가
  무너진 것과 같다 — **판정자 급이 이 작업의 핵심 변수다.**
- 남은 여지: **재랭킹이 아직 천장의 38%만 가져왔다.** 후보 10곳이 전부 채점돼 있어
  "그 10곳에서 최선으로 다섯을 고르면"을 계산할 수 있다.

  ```
  임베딩 상위 5곳          29.7%   nDCG 0.778
  VLM이 고른 5곳           35.4%   nDCG 0.883
  후보 10곳에서 최선(천장)   44.6%   nDCG 0.974
  ```

  그런데 **39장 중 10장(26%)은 후보 10곳에 좋은 곳이 아예 없다** — 재랭킹으로 무엇을
  해도 0%다. 좋은 곳이 있는 29장만 보면 임베딩 40.0% → VLM 47.6% → 천장 60.0%로
  **여지가 +12.4%p** 남는다. 문턱이 가망 없는 질의를 절반쯤 걸러 주지만(문턱 아래
  50% 대 문턱 위 12%), 문턱을 넘고도 좋은 곳이 없는 질의가 3장 있어 그만큼은 헛돈이다.

  **다음에 볼 곳이 둘로 갈린다** — 재랭킹을 더 잘하는 쪽(더 강한 모델·점수 눈금·후보당
  사진 여러 장)과 후보 못을 넓히는 쪽(11~30위에 좋은 곳이 있는지, 채점 200건).
