...
- 운영 전제: Supabase 대시보드에서 익명 로그인을 활성화하고 CAPTCHA 또는 rate limit을
  함께 켠다. 누구나 무제한으로 사용자를 만들 수 있는 엔드포인트라 방치하면 MAU가
  부풀어난다. 익명 사용자 발급은 페이지 로드가 아니라 "게스트로 시작하기" 버튼을
  눌렀을 때만 한다.
- 미결: (1) JWT 검증 방식(JWKS 로컬 검증 vs `GET /auth/v1/user`) 확정, (2) 오래된 익명
  사용자 정리 주기와 참조 행 처리, (3) 이미 계정이 있는 사용자가 게스트 상태에서
  로그인할 때의 데이터 병합 — uid가 달라지는 유일한 케이스로, 로그인 도입 시점에 별도
  결정으로 다룬다.
