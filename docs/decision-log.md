...
- 결정 9: **지금은 사진을 보내지만, 글로 바꾸면 값이 12분의 1이 될 수 있다.** 검색당
  26원을 줄이려고 장소마다 오픈 VLM(Qwen2.5-VL-3B)이 쓴 분위기 설명글을 미리 만들어
  두고 그 글로 비교해 봤다.

  | 비교 방식 | 차이 | p | nDCG | 검색당 |
  | --- | --- | --- | --- | --- |
  | 글 + 코사인 | +0.0%p | 0.977 | 0.701 | 0원 |
  | **글 + Gemini 판정** | **+4.8%p** | 0.146 | 0.787 | **3.2원** |
  | 글 + Qwen2.5-3B 판정 | −0.8%p | 0.754 | 0.764 | 3.4원 |
  | 사진 + Gemini 판정 | +5.6%p | 0.037 | 0.894 | 40.0원 |

  **같은 글인데 판정자만 바꾸니 +0.0%p가 +4.8%p가 된다.** 글에 정보가 없던 것이
  아니라 코사인이 그것을 못 읽었다. 글과 사진의 차이는 −0.8%p(`p = 0.558`)로 갈릴 수
  없는 크기다.

  **그래도 이번에는 사진을 보낸다.** 셋이 걸린다 — 글 쪽은 `p = 0.146`으로 유의하지
  않고(설명글이 다 있는 질의가 25장뿐이다), **nDCG가 0.787 대 0.894로 눈에 띄게
  낮으며**(뽑아오는 곳은 비슷한데 그 안에서 순서를 덜 잘 매긴다), 6,829곳의 설명글을
  미리 만들어야 한다(코랩 T4로 3~4시간, 값은 0원). 켜기로 정해진 뒤에 별건으로 본다.

  **오픈 모델은 판정을 못 한다.** `Qwen2.5-3B`는 −0.8%p로 임베딩 순서보다도 못했고
  29장 중 4장은 답 형식조차 못 맞췄다(3B는 응답 스키마를 걸 수 없다). 확인 21에서
  `flash-lite`가 무너진 것과 같다 — **판정자 급이 이 작업의 핵심 변수다.**
