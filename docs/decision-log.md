...
- 남은 것: 시간을 말하지 않은 턴은 이 설계가 통째로 꺼져 있다
  (`derive_item_range()`가 상수 `(3, 5)`를 돌려주고 배분·판정·안내가 모두
  건너뛰어진다). 실측에서 258~420분 일정이 안내 없이 나갔다 — 별도 결정이
  필요하다. 그리고 후보가 1곳만 오는 경우가 관측됐는데(경복궁 0.2km 세 곳을
  놓치고 2km 밖 한 곳) 폐점 필터·추천 이력 둘 다 배제돼 원인 미확정이다.
