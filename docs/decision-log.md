...
- 결정 8: **보여줄 수보다 넉넉히 보낸다 — 5곳을 보여주고 10곳을 보낸다.** 보내는
  ...
  **상위 5곳 기준으로 직접 쟀다.** 질의 33장을 같은 조건에 놓고 후보 수만 바꿨다.

  | 후보 | 임베딩 | VLM | 차이 | p | 겹침 | 검색당(문턱 0.50) |
  | --- | --- | --- | --- | --- | --- | --- |
  | 6곳 | 26.7% | 29.1% | +2.4%p | 0.128 | 4.6/5 | 19원 |
  | 8곳 | 26.7% | 31.5% | +4.8%p | 0.055 | 3.9/5 | 26원 |
  | **10곳** | 26.7% | **32.1%** | **+5.5%p** | **0.010** | 3.3/5 | **29원** |

  **여유가 클수록 이득이 커지고 10곳에서만 유의하다.** 6곳은 겹침이 4.6/5라 거의
  바뀌지 않는다 — 여유가 1곳뿐이라 재랭커가 할 일이 없다. 12곳은 재지 않았는데
  증가폭이 +2.4 → +0.7로 줄고 있어, 늘려도 잡음 바닥(2.6%p) 안이다.

  처음에는 상위 3곳 기준 측정만 있어 "잰 적 있는 비율"을 옮긴 8곳으로 두었는데,
  직접 재니 10곳이었다. 이 측정에 API 4,169원과 채점 276칸을 썼다.
...
- 해소: **측정과 제품의 어긋남은 상위 5곳 재측정으로 닫혔다.** 확인 15~22가 상위
  ...
  다시 재니 **효과는 살아남았고**(+5.5%p, `p = 0.010`) **후보 수만 8 → 10으로
  바뀌었다.** 문턱 0.50은 그대로 유효했다(호출 73%, `p` 0.010 → 0.008).
...
- 한계: **채점자 한 명의 회차 흔들림이 재려는 효과와 같은 크기다.** 이번 측정에서
  ...
  +5.5%p인데 회차 차이가 그만큼 움직이므로, **후보를 미리 다 채점해 두고 VLM을
