...
- 결정:
  1. 모델은 `google/siglip2-base-patch16-224`(768차원)를 쓴다. `-384`와 비교했으나
     정답표 채점 차이가 평균 +0.030으로 잡음 범위였고(색온도 +0.077은 장소 한 곳
  ...
  3. 분위기 축은 후보 11개를 여섯 단계 검사로 걸러 여덟을 남기고 **다섯을 켠다** —
     안팎 0.992 · 한산함 0.844 · 시대 0.832 · 색온도 0.790 · 세월 0.787(사람 정답표
     77곳 기준 AUC). 끈 셋은 규모(두 사람 일치도가 0.600으로 **사람도 못 정하는
     축**)·정돈(여덟 중 유일하게 모델이 사람보다 뒤처진다)·색감(시대와 +0.47로
     겹친다)이다.
...
- 남은 것:
  ...
  - `정돈` 축(모델 0.600 대 사람 0.700)과 `규모` 축(사람끼리 0.600)은 끈 상태로 둔다.
...
| 2026-08-26 | D-094 신설 — 분위기 임베딩을 조회 계층에 연결(D-087 후속, C). `PlaceMoodRepository` 계약과 `search_place_mood` RPC를 만들어 발화 경로(축 점수 조회)와 사진 경로(최근접 검색)를 붙였다. 재정렬(D)과 축 선택(A)은 범위 밖 |
