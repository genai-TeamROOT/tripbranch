...
**RECOMMEND에는 적용하지 않는다**: 이 절은 INFO(단일 장소 질의) 전용이다.
RECOMMEND 후보에는 직접 조회된 혼잡도만 사용한다. 카페·음식점 후보에 같은 fallback을
적용하면 후보당 API 호출이 최대 2배로 늘어나므로, 혼잡도 데이터 부족으로 추천 품질이나
결과 수 문제가 실제로 확인될 때만 C·D 사전 협의 후 확장을 검토한다.
