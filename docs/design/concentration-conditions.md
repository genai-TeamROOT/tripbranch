...
|------|-----|
| 버전 | v0.6 |
| 상태 | 초안 (Draft) — §2.2/§2.3 D 확인 완료(D-039, 안 B 채택·구현 완료) |
| 최종 수정 | 2026-08-02 |
| 소유 | A (Agent Runtime) |
...
공식의 상세 정의(→ [recommendation-scoring.md](./recommendation-scoring.md), D-039 확정).
...
| **2차 Scoring** | **5개**(1차 상위 결과) | **있음** | "같은 10개를 다시 채점"이 아니라 **후보 집합 자체가 5개로 좁혀진 뒤 새 Feature가 추가된 재채점** — `rerank_with_concentration()`으로 구현 완료(D-039) |
...
### 2.3 Scoring 반영 개요 — ✅ D-039 확정 (1차/2차 구조)
...
- `FakeConcentrationProvider`가 어제/오늘/내일 3일치만 반환해 실제 ~30일 범위를
  ...
  또는 별도 Fixture 필요
- **(2026-08-02 해소, D-039)** §2.2 재검토는 안 B 채택·구현 완료로 종료.
  안 A(v0.4)는 대안으로만 유지.
...
- [recommendation-scoring.md](./recommendation-scoring.md) — `concentration` Scoring Feature 상세 설계 (D-039 확정)
- [agent-runtime-contract.md](./agent-runtime-contract.md) §6 — 혼잡도 보강 흐름 (D-039로 안 B 확정·구현 완료)
...
- [`docs/decision-log.md`](../decision-log.md) D-036 — §3.3의 "혼잡도 fallback: 장소 근접치" 채택 결정, RECOMMEND 미적용·INFO 전용으로 D 확인 완료(2026-08-02), C의 `fetch_info_context()` 구현만 남음. D-037 — §2.2 재검토(1차 Scoring 후 상위 5개 보강 재계산 안) 제안 기록, D-039 — D-037 제안을 D가 확인·채택하고 2차 Scoring 신규 인터페이스를 구현 완료한 기록
