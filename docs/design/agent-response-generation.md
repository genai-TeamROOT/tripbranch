...
|------|-----|
| 버전 | v1 |
| 상태 | §6 1차·2차 구현 완료(2026-07-28). 3차(INFO/COMPARE)는 별도 트랙, 미착수 |
| 최종 수정 | 2026-07-28 |
...
카드 내용을 챗봇 메시지 안에 다시 풀어쓰지 않는다 — 중복이고, D-06 §3.2("문장
내용 자체를 LLM으로 재작문하지 않고 그대로 노출하는 것을 권장")의 취지에도
맞지 않는다. 챗봇 메시지는 카드를 "소개"만 하고, 상세는 카드가 담당한다.
...
| --- | --- | --- | --- |
| RECOMMEND/MODIFY, 추천 있음 | 고정 한 줄 `"이런 곳들을 찾아봤어요:"` | 아니오 | **완료** |
...
├─ intent == GENERAL
│    → llm.generate_general_answer(topic, original_question) 실제 호출(유일한 LLM 신규 호출)
...
│    └─ recommendations가 비어 있으면 no_data 템플릿, 있으면 고정 wrapper
└─ intent in (INFO, COMPARE)
     → "아직 준비 중" 임시 안내(§3, §6 3차 — 별도 트랙)
```

실제 LLM 호출이 새로 늘어나는 경우는 **GENERAL 하나뿐**이라, `try_agent_runtime.py`로
확인한 RECOMMEND/MODIFY 응답 시간(14~30초대)에는 영향이 없다. 실제 확인:
`run_agent()`로 RECOMMEND/GENERAL/OUT_OF_SCOPE 3종을 실제 Gemini로 호출해
`message` 필드가 각각 의도대로 채워지는 것을 확인했다(2026-07-28).

---

## 5. 잠정 결정 (팀 전체 확인 없이 확정, 2026-07-28)

1. **RECOMMEND/MODIFY wrapper**: 고정 한 줄(`"이런 곳들을 찾아봤어요:"`)로 확정.
   조건별 분기 없음.
...
| 2차 | GENERAL 답변 생성 | 있음(신규) | **완료** |
| 3차(별도 트랙) | INFO/COMPARE 실제 답변 | TBD | 미착수 — C/D와 Context 계약 재협의 먼저 필요(A-C 계약 §3, §7) |

**FakeLLMProvider는 이 작업에서 갱신하지 않았다** — `generate_general_answer()`가
`app/providers/stub.py`에는 없다. 테스트에서 GENERAL 분기를 검증할 땐 로컬 테스트
더블(`tests/test_agent_runtime.py::_LLMProviderWithGeneralAnswer`,
`tests/test_response_composer.py::_StubLLM`)을 쓴다.
