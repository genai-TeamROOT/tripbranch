/*
 * 역할: 담아둔 장소가 있을 때 헤더 오른쪽에서 "N곳 일정 짜기"를 띄운다.
 * 입력: 편성 요청 콜백, 진행 중 여부, 언어.
 * 출력: 담은 곳이 없으면 아무것도 그리지 않고, 있으면 칩 버튼 하나.
 * 호출 시점: ChatPage가 AppHeader의 trailing 자리에 넣는다.
 *
 * **하단 바(SavedPlacesBar)를 대신한다**(2026-09-08). 그 바는 입력창 바로 위에
 * 있어서, 하트를 누른 뒤 맨 아래까지 내려가야 보였다 — 카드를 보며 담는 동안에는
 * 정작 안 보인다. 헤더는 sticky라 어디에 있든 눈에 들어온다.
 *
 * 개수를 문구에 담는다("3곳 일정 짜기"). 개수와 동작을 따로 두면 칩이 둘이 되어
 * 헤더가 좁아지고, 어차피 담긴 곳 전부로 편성하므로 두 값이 늘 같은 것을 가리킨다.
 *
 * **모양은 위치 칩과 같다**(사용자 결정, 2026-09-08). 처음에는 브랜드 색을 채워
 * 동작임을 드러내려 했는데, 헤더에 무게가 다른 칩 둘이 놓이면 띠가 어수선했다.
 * 지금은 같은 프로스티드에 아이콘만 브랜드 색이라 위치 칩과 한 줄로 읽힌다 —
 * 두 칩의 클래스가 같아야 하므로 한쪽을 바꾸면 다른 쪽도 봐야 한다(AppHeader).
 *
