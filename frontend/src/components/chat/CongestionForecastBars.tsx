    ...
    <section className="border-t border-border px-4 py-3">
      <div className="flex items-baseline justify-between gap-2">
        ...
        {card.population_current_level && (
          <span className="text-xs text-muted">현재 {card.population_current_level}</span>
        )}
      </div>
      ...
      >
        {card.population_current_level && (
          <div className="flex min-w-0 flex-1 flex-col items-center gap-1 border-r-2 border-dashed border-border pr-1.5">
            {(() => {
              ...
              return (
                <div
                  className={`flex h-16 w-full items-end rounded-t px-0.5 ring-2 ring-inset ring-ink ${color.track}`}
                >
                  <div
                    className={`w-full rounded-t ${color.bar}`}
                    style={{ height: `${height}%` }}
            ...
            <span className="truncate text-[10px] font-semibold text-ink">현재</span>
          </div>
        )}
        {forecasts.map((forecast) => {
          ...
          return (
            ...
            >
              ...
              <span className="truncate text-[10px] text-muted">
                {hourLabel(forecast.forecast_at)}
              </span>
            </div>
          );
        })}
