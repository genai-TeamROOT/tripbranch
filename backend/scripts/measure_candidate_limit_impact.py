...
D-110에서 후보 한도를 10 → 30으로 올릴 때 쓴 측정을 하나로 합친 것이다. 한도를
...
D-110 작업 중에 Supabase `places`에서 후보를 뽑는 스크립트로 재다가 두 번 헛짚었다 —
...
1. **pool** — 시각 x 한도별 하드 필터 통과 수. 보충 조회를 넣은 값과 뺀 값을 함께
   낸다. **보충을 빼고 보면 밤 시간대를 과소평가한다** — 실제로 D-110 중간에 "9시·21시에
