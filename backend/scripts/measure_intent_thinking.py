...
#
# D-061로 코드가 인텐트 분류에 모델별 예산을 싣게 된 뒤에도 이 우회는 남는다 —
# `--budget`은 kwargs를 덮어쓰므로 코드가 정한 값 대신 측정하려는 값이 실린다.
# 코드에 박힌 값과 다른 예산을 재는 것이 이 스크립트의 존재 이유라 그대로 둔다.
# `--budget`을 생략하면 덮어쓰지 않으므로 코드가 정한 값(D-061)이 그대로 나간다.
# 즉 D-061 이후로 **생략은 "모델 기본값"이 아니라 "코드가 정한 값"이다.** 모델 기본값
# 자체를 재려면 `_INTENT_THINKING_BUDGETS`에서 그 모델을 빼고 돌려야 한다.
