...
async def test_classify_and_schedule_route_to_their_respective_model_groups() -> None:
    ...
    그 예외 때문에 fast 경로의 thinking 끄기가 무효화돼 있었다 — D-074에서 제거했다.
    """
    provider = RealGeminiProvider(
        api_key="dummy",
    ...
    # 두 모델 모두 MINIMAL이 실린다. Flash-Lite만 생략하던 예외는 없다(D-074).
...
async def test_zero_budget_is_never_sent_as_a_number() -> None:
    ...
    실제로 fast 모델이 gemini-3.5-flash-lite로 바뀐 뒤 그 일이 일어났다(D-074 —
