"""지원 지역(서울 12개 구, SUPPORTED_DISTRICTS 참고) 좌표 판정을 고정한다.

경계 상자로는 갈리지 않던 지점이 있어 폴리곤을 쓴다 — 종로구는 남북으로 길쭉하고
북악산 쪽으로 굽어 있다.

지원 구가 네 곳으로 늘면서 명동·서울역이 "밖"에서 "안"으로 바뀌었다(둘 다 중구다).
2026-08-25에 여덟 곳이 더 늘면서(D-082) 청량리역(동대문구)·건대입구역(광진구)도
같은 이유로 "안"으로 옮겼다. 새로 생긴 경계인 서대문·강동·구리·남양주·고양 쪽
지점을 밖에 두어, 확장이 인접 구로 새는지 잡는다.
