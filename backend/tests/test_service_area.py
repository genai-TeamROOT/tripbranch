"""지원 지역(서울 16개 구, SUPPORTED_DISTRICTS 참고) 좌표 판정을 고정한다.
...
홍대입구역(마포구)·신촌역(서대문구)도 같은 이유로 "안"으로 옮겼다. 새로 생긴
경계인 영등포·구로·강동·구리·남양주·고양 쪽 지점을 밖에 두어, 확장이 인접 구로
새는지 잡는다.
...
_INSIDE = {
    ...
    # 구로구
    "신도림역": (37.5088, 126.8912),
    "구로디지털단지역": (37.4850, 126.9016),
    # 금천구
    "가산디지털단지역": (37.4816, 126.8826),
    "독산역": (37.4661, 126.8886),
    # 영등포구
    "여의도역": (37.5216, 126.9243),
    "영등포역": (37.5157, 126.9070),
    # 동작구
    ...
    "교대역": (37.4935, 127.0143),
    # 강남구
    "삼성역": (37.5089, 127.0631),
    "선릉역": (37.5044, 127.0490),
    # 송파구
    "잠실역": (37.5133, 127.1001),
    "석촌호수": (37.5087, 127.1064),
    # 강동구
    "천호역": (37.5385, 127.1237),
    "암사역": (37.5501, 127.1279),
}
_OUTSIDE = {
    # 서울 25개 구가 전부 지원 범위가 되면서(2026-08-29) 서울 안에는 "밖"이 하나도
    # 남지 않는다. 그래서 여기는 인접 경기 시와 원거리 대조군만 둔다 — 확장이
    # 시계를 넘어가지 않는지 잡는 것이 이제 이 묶음의 유일한 역할이다.
    #
    # 각 방향으로 하나씩 둔다. 한쪽만 두면 그 방향의 폴리곤만 지켜진다.
    # 동 — 구리·남양주(중랑·노원·강동과 접한다)
    "구리역": (37.5991, 127.1397),
    "남양주시청": (37.6360, 127.2165),
    # 서 — 부천·김포(강서·양천과 접한다)
    "부천역": (37.4844, 126.7830),
    "김포시청": (37.6152, 126.7157),
    # 남 — 광명·과천·안양·성남(구로·금천·관악·서초·강남과 접한다)
    "광명역": (37.4165, 126.8846),
    "과천역": (37.4332, 126.9968),
    "안양역": (37.4018, 126.9227),
    "성남시청": (37.4200, 127.1265),
    # 북 — 고양·의정부(은평·도봉·강북과 접한다)
    "화정역": (37.6349, 126.8355),
    "의정부역": (37.7383, 127.0464),
    # 원거리 대조군 — 판정이 통째로 무너지면 여기서 먼저 걸린다.
...
# 구로·금천·영등포·동작·관악·서초구도 같은 기준(2026-08-28 확인, 폴리곤 오차 0.33% 이내).
# 강남·송파·강동구도 같은 기준(2026-08-29 확인, 폴리곤 오차 0.23% 이내).
_OFFICIAL_AREA_KM2 = {
    "종로구": 23.91,
    "중구": 9.96,
    "용산구": 21.87,
