...
# 개수: 직전 턴에 보여준 세 개.
#
# **넓히면 모델이 오히려 규칙을 어긴다.** 2026-09-09 실측 — 같은 장소에 대한 질문을 네 개
# 넘겨주자 모델이 목록에 글자 그대로 있는 문구를 다시 만들어 돌려줬고(세 개 중 둘),
# 아래 _clean()이 그걸 걷어내니 화면에 붙는 버튼이 한 개로 줄거나 아예 없어졌다. 목록에
# 관계없는 문구 네 개를 넣었을 때는 멀쩡히 세 개를 만들었으므로, 개수가 아니라 "같은
# 장소에 대한 질문이 쌓이는 것"이 모델을 무너뜨린다.
#
