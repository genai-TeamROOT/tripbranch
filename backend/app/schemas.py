...
#
# 개수: 최근 열 개. 대략 세 턴 분량이다.
#
# 2026-09-09 실측(같은 장소를 두고 이어가는 대화, 제외 0/3/5/7개) — 어느 구간에서도 모델이
# 목록에 있는 문구를 다시 만들어내지 않았고, 다섯 개까지는 세 개를 그대로 채웠다. 일곱 개에서
# 둘로 줄지만 그건 규칙을 어긴 게 아니라 소재가 떨어진 것이고, 그 둘도 아직 안 권한 방향으로
# 갔다(도로 소통, 공영주차장). 열 개가 전부 같은 장소 질문으로 차는 대화는 드물다.
#
# **이 값을 셋으로 줄였다가 되돌렸다.** 줄인 근거였던 "넓히면 모델이 규칙을 어긴다"는 관측이
# 잘못이었다 — 그때 로컬 프롬프트가 안 읽히고 있었다(LANGFUSE_PROMPTS_ENABLED가 켜져 있어
# already_suggested 규칙이 없는 원격 1.4.0으로 돌았다). 규칙이 실제로 실린 상태에서 다시
# 재니 위반이 한 건도 없었다. 앞으로 이 값을 다시 조정할 때는 그 스위치부터 확인한다.
#
