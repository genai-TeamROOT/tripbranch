...
**바꾸지 않은 것**: 동행 표현이 `taste_query`에 남아도 실제 취향 근거 RAG
검색에서 도움이 되는지(예: "친구들이랑" 문구로 리뷰가 실제로 걸리는지)는 이번에
측정하지 않았다 — TP-128에서 동행 단독 발화 기준으로는 확인했지만(명소 8/8,
식당 7/8), 이번처럼 취향 단어와 함께 오는 복합 발화 기준으로는 별도 측정이
필요하다.
