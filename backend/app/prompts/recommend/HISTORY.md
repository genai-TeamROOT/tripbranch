...
| 2.1.0 | 2026-08-21 | (커밋 대기) | `recommend.extract` (2.0.0 → 2.1.0) | 혼잡도 표현(조용한·한적한·붐비는·북적이는·시끌벅적)을 `taste_query`에서 제외하고 `concentration_intent` 전용으로 둠. 대표 예시 "혼자 조용히 쉴 만한" 교체 | "조용한/한적한" 발화가 `taste_query`와 `concentration_intent=AVOID`를 동시에 채워, 한 선호가 두 축에서 가중치 0.30(0.15+0.15)을 가져가던 것을 막는다(단일축 선호는 0.15) | 실 LLM 2회 검증(혼잡도 단어 누출 0/6), 전체 테스트 2142 통과·스냅샷 갱신, 커밋·PR 대기 |
...
### 2.1.0 — 혼잡도 단어를 `taste_query`에서 분리 (2026-08-21)
...
2.0.0 대표 예시 "혼자 조용히 쉴 만한 곳 → 혼자 조용히 쉴 만한"이 그 동작을
