...
class Settings(BaseSettings):
    ...
    # GENERAL 답변을 LangGraph 그래프로 태울지 여부(1단계,
    # docs/design/langgraph-adoption.md §6.1). 기본 on이지만, 이관은 출력이
    # 같아야 하는 작업이라 문제가 보이면 이 값 하나로 즉시 기존 경로로 되돌린다 —
    # off로 두면 그래프를 아예 부르지 않고 compose_chat_message()를 직접 호출한다.
    use_langgraph_general: bool = True
