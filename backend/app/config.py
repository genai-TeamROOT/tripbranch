...
class Settings(BaseSettings):
    ...
    # 10장의 이득은 정확히 +0.0%였다.
    #
    # **10곳은 상위 5곳 기준으로 직접 잰 값이다**(TP-214). 질의 33장을 같은
    # 조건에 놓고 후보 수만 바꿨다.
    #
    #     후보 6곳    26.7% → 29.1%   +2.4%p   p = 0.128   여유 1곳
    #     후보 8곳    26.7% → 31.5%   +4.8%p   p = 0.055   여유 3곳
    #     후보 10곳   26.7% → 32.1%   +5.5%p   p = 0.010   여유 5곳
    #
    # 여유가 클수록 이득이 커지고 10곳에서만 유의하다. 12곳은 재지 않았는데,
    # 증가폭이 +2.4 → +0.7로 줄고 있어 늘려도 잡음 바닥(2.6%p) 안이다.
    #
    # 토큰은 후보 1곳당 1,083개씩 붙고 고정분이 1,279개다(실측, 사진 해상도는
    # 무관). 문턱 0.50까지 걸었을 때 검색당 6곳 19원 · 8곳 26원 · 10곳 29원이다.
