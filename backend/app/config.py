...
class Settings(BaseSettings):
    ...
    # 순서만 바꾸므로 뽑히는 곳 자체가 그대로인데, 우리가 쓰는 지표(상위 N칸 중
    # 몇 칸이 "비슷하다"인가)는 순서와 무관하다. 실제로 후보 5곳 판에서 뽑힌 곳이
    # 그대로였던 질의 10장의 이득은 정확히 +0.0%였고, 뽑힌 곳이 바뀐 29장에서만
    # +8.0%가 나왔다(TP-213). 이 값을 5로 내리려면 그 사실부터 뒤집어야 한다.
    #
    # 토큰은 후보 1곳당 1,083개씩 붙고 고정분이 1,279개다(실측). 사진 해상도는
    # 영향이 없다. 문턱까지 걸었을 때 검색당 5곳 16원 · 8곳 24원 · 12곳 35원이다.
    #
    # **8곳인 이유는 잰 적 있는 비율을 맞춘 것이다.** 측정은 "5곳 보내고 3곳 본다"
    # (1.67배)에서 +6.0%p였고, 5곳을 보여줄 때 같은 비율이 8곳(1.60배)이다.
    # 12곳(4.00배)은 +6.8%p로 더 컸지만 비용이 1.4배다.
    #
    # **상위 5곳 기준으로 직접 잰 값은 아직 없다.** 지금까지의 측정은 전부 상위
    # 3곳 기준이라, 재측정 뒤 확정한다. 그 3곳은 의도된 값이 아니다 — 사진 검색은
    # 한 번 5곳으로 맞췄다가 이후 실험들이 근거 없이 3곳으로 돌아갔다.
    place_mood_rerank_candidate_count: int = 8

    # 1위 유사도가 이 값 미만이면 VLM을 부르지 않는다(TP-213 확인 22).
    #
    ...
    #
    # 문턱 0.50에서 호출이 74%로 줄고 품질은 전부 부를 때와 같았다. 0.55부터는
    # 도움이 되는 질의까지 잘라낸다.
    #
    # **이 값은 place_mood_mean_center_enabled가 켜져 있을 때의 눈금이다.**
    # 평균을 빼지 않으면 유사도 분포가 달라져 이 문턱이 뜻을 잃는다.
    #
    # 질의 39장·채점자 한 명으로 고른 값이라 0.50과 0.525를 가리지 못했다.
    # 안전한 쪽을 골랐고, 실사용 로그가 쌓이면 다시 잰다.
    place_mood_rerank_min_top_similarity: float = 0.50
